class FreeShippingMeter extends HTMLElement {
    ...
    calculateProgress(cart) {
        ...
        let freeShipBar = (cartTotalPriceRounded * 100) / FreeShippingMeter.freeshipPrice;

        if (cartTotalPriceRounded >= FreeShippingMeter.freeshipPrice) {
            freeShipBar = 100;
        } else {
            freeShipBar = Math.floor(freeShipBar);
        }
    ...
    getText(cartTotalPrice, freeShipBar) {
        ...
        if (cartTotalPrice == 0) {
            ...
            this.progressBar.classList.remove('progress-hidden');
            const remainingPrice = (FreeShippingMeter.freeshipPrice - cartTotalPrice).toFixed(2); // Preserve decimal places
